...
export default function EditarPerfilp({ navigation, route }) {
  ...
  const [datanascimento, setDatan] = useState(datanascimentoe ? new Date(datanascimentoe) : null);
  const [genero, setGenero] = useState(generoe);
  const [especialidade, setespecialidade] = useState(espe);
  const [experiencia, setexperiencia] = useState(expe);
...
if (Platform.OS === 'web') {
  return (
    <SafeAreaView style={stylesWeb.safeArea}>
        ...
        <View style={stylesWeb.right}>
          <View style={stylesWeb.form}>
            ...
            <TouchableOpacity style={stylesWeb.input}>
              <input
                ...
                style={{
                  ...
                  textAlign: 'center',
                  color: datanascimento ? '#4CD964' : '#b3b3b3'
                }}
                value={datanascimento ? datanascimento.toISOString().split('T')[0] : ''}
                onChange={(e) => {
                  const parts = e.target.value.split("-");
                  setDatan(new Date(Number(parts[0]), Number(parts[1]) - 1, Number(parts[2])));
                }}
                min="1900-01-01"
            ...
            <TouchableOpacity style={stylesWeb.input} onPress={() => setShowGenderModal(true)}>
              <Text style={{ color: genero ? '#4CD964' : '#b3b3b3' }}>{genero || 'Gênero'}</Text>
            </TouchableOpacity>

            <TouchableOpacity style={stylesWeb.input} onPress={() => setshowexperiencia(true)}>
              <Text style={{ color: experiencia ? '#4CD964' : '#b3b3b3' }}>{experiencia || 'Anos de experiência'}</Text>
            </TouchableOpacity>

            <TouchableOpacity style={stylesWeb.input} onPress={() => setshowtrabalho(true)}>
              <Text style={{ color: especialidade ? '#4CD964' : '#b3b3b3' }}>{especialidade || 'Área de trabalho'}</Text>
            </TouchableOpacity>
      ...
      <Modal visible={showexperiencia} transparent animationType="slide" onRequestClose={() => setshowexperiencia(false)}>
        <View style={stylesWeb.modalContainer}>
          <View style={stylesWeb.modalContent}>
            <Text style={stylesWeb.modalTitle}>Selecione a Experiência</Text>
            <FlatList data={experent} keyExtractor={item => item} renderItem={({ item }) => (
      ...
      <Modal visible={showtrabalho} transparent animationType="slide" onRequestClose={() => setshowtrabalho(false)}>
        <View style={stylesWeb.modalContainer}>
          <View style={stylesWeb.modalContent}>
            <Text style={stylesWeb.modalTitle}>Selecione a Área de Trabalho</Text>
            <FlatList data={work} keyExtractor={item => item} renderItem={({ item }) => (
              <TouchableOpacity style={stylesWeb.modalItem} onPress={() => { setespecialidade(item); setshowtrabalho(false); }}>
                <Text style={stylesWeb.modalText}>{item}</Text>
              </TouchableOpacity>
            )} />
          </View>
